...
def search_menu_items(
    ...
    only_in_stock: Optional[bool] = None,
    ...
    if menu_id:
        ...
        menu_items = load_all()
    if only_in_stock:
        menu_items = [r for r in menu_items if r["in_stock"]]
    if name:
    ...
    return [MenuItem(**r) for r in menu_items[start:end]]
...
def update_menu_item(menu_item_id: str, payload: MenuItemUpdate) -> MenuItem:
    ...
    for idx, m in enumerate(menu_items):
        if m.get("id") == menu_item_id:
            ...
            updated_description = cast(
                str,
                payload.description.strip() if payload.description is not None else str(m.get("description")),
            )
            ...
            updated_menu_id = cast(str, str(m["menu_id"]))
            updated = MenuItem(
                id = menu_item_id,
                name=updated_name,
